...
$sql_SELECT = 'SELECT MAX(no) as mx FROM member_list';
$result = mysql_query($sql_SELECT, $s);
$row =  mysql_fetch_assoc($result);
$sql_id = (int)(((string)mt_rand(1, 9)) . ((string)($row['mx'] + 1)));

$sql_INSERT = <<<EOSql
INSERT INTO `member_list`
    (`no`, `id`, `name_uji`, `name_na`, `name_uji_yomi`, `name_na_yomi`, `mail_address`,
    `birth`, `age`, `blood_type`, `course_name`, `course_charge`, `special_report`)
VALUES (NULL, '{$sql_id}', '{$sql_name_uji}', '{$sql_name_na}', '{$sql_name_uji_yomi}',
    '{$sql_name_na_yomi}','{$sql_mail_address}','{$sql_birth}', '{$sql_age}',
    '{$sql_blood_type}', '{$sql_course_name}', '{$sql_course_charge}', '{$sql_special_report}')
EOSql;

echo $sql_INSERT;

$table = mysql_query($sql_INSERT, $s);
